              ...
              this.elements[i].addEventListener(event, function(eventObject){
                ...
                for(var k = 0; k < gq_events[id][selector][event].length; k++){
                  ...
                  if(eventObject.target.matches(selector)){
                    ...
                    eventObject = Object.assign({}, eventObject, newEvent);

                    gq_events[id][selector][event][k](eventObject);
                  // }else if(eventObject.target.parentNode.matches(selector)){
                  //   GQLogger.debug('matched on parent', eventObject.target);
                  //   var newEvent = {
                  //     target: eventObject.target.parentNode,
                  //     originalEvent: eventObject,
                  //     preventDefault: function(){
                  //       this.originalEvent.preventDefault();
                  //     }
                  //   };
                  //   eventObject = Object.assign({}, eventObject, newEvent);
                  //   GQLogger.debug(eventObject.target);
                  //   gq_events[id][selector][event][k](eventObject);
                  }else{
                    GQLogger.debug('unmatched selector:', selector);
                    var currentTarget = eventObject.target.parentNode;
                    while(currentTarget){
                      if(currentTarget && currentTarget.matches && currentTarget.matches(selector)){
                        GQLogger.debug('matched on parent', currentTarget);
                        var newEvent = {
