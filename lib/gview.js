class GViewApp {
	...
	changeView(viewName, data){
		...
		Logger.debug('view - currentController', App.currentController)
...
class View {
	...
	render(params){
		...
		if(App.cache.views.hasOwnProperty(this.name)){
			Logger.debug('Loading from cache...');
			var t = App.cache.views[this.name];

			this.afterLoad(t, params);
	...
	afterLoad(t, params){
		...
		Logger.debug('cv', App.currentView)
		var loadedController = document.getElementById('gview-controller-script-' + App.currentView);
		if(!loadedController){
			Logger.debug('not loaded');
			App.injectScript('./controllers/' + this.referenceName + '.js', 'gview-controller-script-' + this.name, () => {
				Logger.debug('controller js loaded', Strings.snakeToClass(this.name),  App.componentClasses[Strings.snakeToClass(this.name)]);
				App.currentController = App.componentClasses[Strings.snakeToClass(this.name)];
			});
		}else{
			Logger.debug('loaded')
			App.currentController = App.componentClasses[Strings.snakeToClass(this.name)];
		...
		Handlebars.registerHelper(name, ((options) => {
			...
			for(var v = 0; v < this.events.length; v++){
				...
				$('#GViewContainer').on(event.event, '#' + this.id, ((e) => {
					...
					if(this.classEvents.indexOf(event.event) > -1){
						App.componentClasses[className][event.event](e);
					}
					Logger.debug('currentController', App.currentController);
					if(App.currentController && App.currentController[event.function]){
						App.currentController[event.function](e);
					}else{
						Logger.error('Method [' + event.function + '] for event [' + event.event + '] undefined');
					}
				}).bind(this));
			}
			...
			for(var z = 0; z < this.classEvents.length; z++){
				if(attachedEvents.indexOf(this.classEvents[z]) == -1){
					$('#GViewContainer').on(this.classEvents[z], '#' + this.id, ((e) => {
						var componentType = Strings.snakeToClass(this.id.replace('gview-component-', '').replace(/[0-9]/g, ''));
						App.componentClasses[className][e.type](e);
					}).bind(this));
				}
			}

			if(App.componentClasses[className].afterRender) setTimeout(() => {App.componentClasses[className].afterRender($('#' + this.id))}, 2);
